...
import {transferMoney} from '../game/action';
import { GameValue } from '../config/game';

export default function DialogSelectPlayer(props) {

    actionSelect = (item) => {
        let info = {
            playerId:item.playerId,
            amount:props.amount.loan + props.amount.balance
        };  
        transferMoney(info);
        GameValue.cashhand -= props.amount.balance;
        GameValue.rm_loan -= props.amount.loan;
        props.backMain();
    }
        ...
        <View style={{backgroundColor:'rgba(0, 0, 0, 0.8)',position:'absolute',left:0,top:0,right:0,bottom:0}}>
            <View style={{justifyContent:'center',alignItems:'center',padding:20,width:'100%',flex:1}}>
                <View style={{width:'100%'}}>
                    <Text style={{fontSize:25,color:'#D9D8D6',textAlign:'center',fontFamily:'Montserrat'}}>Please select Player to send ${priceFormat(props.amount.loan + props.amount.balance)}</Text>
                    <ScrollView>
                        <View>
                            {
